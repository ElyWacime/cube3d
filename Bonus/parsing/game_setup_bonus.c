...
/*   Updated: 2025/02/21 16:44:49 by skamroun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_for_images(t_var *var)
{
	...
	fd = open("./Bonus/textures/door.png", O_RDONLY);
	if (fd < 1)
	{
		write(2, "Error 69\nFile Not Found!", 25);
		quit_program(var);
	}
	fd = open("./Bonus/textures/gunImageFire.png", O_RDONLY);
	if (fd < 1)
	{
		write(2, "Error 69\nFile Not Found!", 25);
		quit_program(var);
	}
	fd = open("./Bonus/textures/gunImagePreFire.png", O_RDONLY);
	if (fd < 1)
	{
		write(2, "Error 69\nFile Not Found!", 25);
		quit_program(var);
...
void	load_pictures(t_var *var)
{
	...
	var->door = mlx_load_png("./Bonus/textures/door.png");
	var->gunfire = mlx_load_png("./Bonus/textures/gunImageFire.png");
	var->gunprefire = mlx_load_png("./Bonus/textures/gunImagePreFire.png");
}
