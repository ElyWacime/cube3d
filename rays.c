...
void  cast_up(t_var *var)
{
    ...
    line.bx = -tan(from_deg_to_rad(var->player.angle + 30)) * (fabs(player[1] - line.by)) + player[0];
    ...
    {
        tmpby -= 32;
        tmpbx = -tan(from_deg_to_rad(var->player.angle + 30)) * (fabs(player[1] - tmpby)) + player[0];
