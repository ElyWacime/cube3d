...
void    sub_init_mini_map_system(t_var *var, t_imm *imm)
{
    while (imm->j < imm->end.y && imm->map_y < 100 && imm->j < var->mini_height)
    {
        ...
        {
            if (imm->i > ft_strlen(var->map[px_to_map_grid(imm->j)]) * SQUARE_SIZE)
                break;
            if (var->player.position.x - 2 < imm->i && imm->i < var->player.position.x + 2
                && var->player.position.y - 2 < imm->j && imm->j < var->player.position.y + 2)
